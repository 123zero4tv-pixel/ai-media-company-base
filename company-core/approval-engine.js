...
function approveCeoTask(taskId, comment=''){
  ...
  const approval=(state.approvals||[]).find(a=>a.task_id===taskId&&a.status==='PENDING');
  ...
  resolved.status='APPROVED'; resolved.comment=comment; resolved.resolved_at=now();
  ...
  saveState(fresh);
  return {task:result.task,approval:resolved,state:loadState()};
}
function rejectCeoTask(taskId, reason='Owner rejected CEO recommendation'){
  ...
  const fresh=loadState();
  const resolved=fresh.approvals.find(a=>a.approval_id===approval.approval_id);
  resolved.status='REJECTED'; resolved.comment=reason; resolved.resolved_at=now();
  fresh.company.pending_approval_ids=(fresh.approvals||[]).filter(a=>a.status==='PENDING').map(a=>a.approval_id);
